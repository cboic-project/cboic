...
*Setup instructions for a gitian build of Cboic using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cboic
...
Clone the git repositories for cboic and gitian and then checkout the cboic version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cboic-project/cboic.git
cd cboic
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cboic repository
...
To build Cboic (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cboic repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cboic/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cboic-project/cboic
...
For example:
```bash
URL=https://github.com/user/cboic.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit cboic=${COMMIT} --url cboic=${URL} ../cboic/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cboic=${COMMIT} --url cboic=${URL} ../cboic/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cboic=${COMMIT} --url cboic=${URL} ../cboic/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cboic-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cboic-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cboic-build.assert
```
...
[cboic/gitian.sigs](https://github.com/cboic-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
